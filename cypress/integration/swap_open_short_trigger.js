    ...
    it('访问交易所页面，验证合约委托开空功能', () => {
      cy.visit('http://localhost:8080/#/login').then(() => {
        ...
        cy.visit('http://localhost:8080/#/swap/btc_usdt').then(() => {
            cy.wait(3000);
            cy.get(".short_open_type").click().should('be.visible');
            cy.contains('.font-light.text-white.truncate','Trigger Order').click();
            cy.get(".short_open_trigger_price").should("exist").type("29000");
            cy.get(".short_open_price").should("exist").type("29000");
            cy.get(".short_open_vol").should("exist").type("1");
            cy.get('.open_short_order').should("exist").click();
            cy.wait(3000);
